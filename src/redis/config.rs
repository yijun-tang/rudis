...
use crate::util::{yes_no_to_bool, LogLevel};
    ...
    pub fn load_server_config(&mut self, filename: &str) {
        ...
        if filename.eq("-") {
            ...
            if let Ok(f) = File::open(filename) {
                ...
                // redis_log(LogLevel::Warning, "Fatal error, can't open config file");
                exit(1);
            }
        }
            ...
            if let Ok(line) = line {
                ...
                match &argv[0].to_ascii_lowercase()[..] {
                    ...
                    "dir" if argc == 2 => {
                        ...
                        if ret_val == -1 {
                            // redis_log(LogLevel::Warning, "Can't chdir to '{}': {}", argv[1], err);
                            exit(1);
                        }
                    },
                    "loglevel" if argc == 2 => {
